...
export class SaveSellersPage {
  ...
  sellers: sellersModel = new sellersModel;
  ...
  textCategories(){
    ...
    //this.form.value.category_ids=text;
     this.userProvider.check_User().then( data => {
      this.sellers.user_id = data;
      console.log('Checkbox data:', this.sellers);
      this.sellers.category_ids = text,
      console.log('Checkbox data:', this.sellers.category_ids);
    } );
  }
  createSeller(){
    let loading = this.alert.loading('Cargando...');
    ...
    this.sellersProvider.createSellers(this.sellers).subscribe(
      data =>      {
        data = JSON.parse(data['_body']);
        loading.dismiss();
