...
export class SaveIndependientsPage {
  ...
  independients: independientsModel = new independientsModel;
  ...
  textCategories(){
    ...
    //this.form.value.category_ids=text;
     this.userProvider.check_User().then( data => {
      this.independients.user_id = data;
      console.log('Checkbox data:', this.independients);
      this.independients.category_ids = text,
      console.log('Checkbox data:', this.independients.category_ids);
    } );
  }
  createIndependient(){
    let loading = this.alert.loading('Cargando...');
    ...
    this.independientsProvider.createIndependient(this.independients).subscribe(
      data =>      {
        console.log(data);
        data = JSON.parse(data['_body']);
        loading.dismiss();
  ...
  setImgShop(image){
    ...
    .then(photoURL => {
        this.independients.avatar = photoURL;
    })
  }
